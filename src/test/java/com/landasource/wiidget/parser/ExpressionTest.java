...
public class ExpressionTest {
    ...
    public void testSimpleStrings() {
        ...
        for (char i = Character.MIN_VALUE; i < Character.MAX_VALUE; i++) {
            ...
            assertExpression(string, "\"" + string + "\""); // check the string
    ...
    public void testMapGet() {
        wiidgetContext.set("modes", new DataMap());
        wiidgetContext.set("var", "1");

        assertExpression(" btn-danger", "\" \" + ({\"OFF\" : \"btn-danger\", \"ON\" : \"btn-success\", \"AUTO\" : \"btn-info\"}.get(modes.get(var) ~ \"OFF\"))");
    }
    ...
    protected static ExpressionEvaluator createEvaluator(final Context wiidgetContext) {
        ...
        final EvaluationContext evaluationContext = new EvaluationContext(new MockImportContext(engine), engine, new TemplateProcessor(engine));
