 ...
 */
public class TemplateProcessor extends WiidgetView implements StatementProcessor {

    /** */
    public static final String DEFAULT_VALUE_ATTRIBUTE = "value";
     ...
     */
    private final Wiidget ownerView;
    ...
    /**
     * @param engine specified factory
     */
    ...
    /**
     * @param engine specified factory
     * @param owner  the owner wiidget
     */
    public TemplateProcessor(final Engine engine, final Wiidget owner) {
        super(engine);
     ...
     *
     * @param importDeclaration imports in template
    ...
    /**
     * @param statementDeclaration statements
     * @throws ParserException when cannot parse statement
     */
    @Override
    public void processStatements(final List<StatementDeclarationContext> statementDeclaration) throws ParserException {
    ...
    /**
     * @param statementDeclarationContext statement
     * @throws ParserException when cannot parse statement
     */
    ...
    /**
     * @param controlStatementContext control statements
     * @throws ParserException when cannot do control
     */
    private void processControl(final ControlStatementContext controlStatementContext) throws ParserException {
        ...
        final ElseControlContext elseControl = controlStatementContext.elseControl(); // this
        // will
        // be
        // used
        // everywhere
    ...
    /**
     * @param foreachControl foreach declaration
     * @param bodyContext    body of loop
     * @throws ParserException when cannot iterate over value
     */
    private void processForeach(final ForeachControl foreachControl, final WiidgetBodyContext bodyContext) throws ParserException {
        ...
        final List<Pair<Object, Object>> entrySet = iterator.entrySet(); // get
        // the
        // iterable
        // list
    ...
    /**
     * @param ifControl   statement
     * @param bodyContext body of context
     * @param
     * @throws ParserException when cannot process body
     */
    ...
    /**
     * @param foreachControlContext foreach
     * @param elseControl           the optional else
    ...
    /**
     * @param ifControlContext   if control
     * @param elseControlContext the optional else
     * @return control object
     * @throws ParserException when the expression is not a boolean
     */
     ...
     *
     * @param declarationContext wiidget declaration
     * @throws ParserException when cannot create wiidget or declaration is illegal
     */
    private void processWiidget(final WiidgetDeclarationContext declarationContext) throws ParserException {
        ...
        if (null == stringWiidgetName) {
            ...
            if (null == expressionWiidgetName) {
                ...
                if (null == wiidgetResource) {
                    // throw new UnknownWiidgetTypeException(declarationContext,
                    // "Unknown wiidget alias: " + alias);
            ...
            final String tagName = new StringDeclaration(stringWiidgetName).getContent(getEngine());
            wiidget = createRawWiidget(tagName, wiidgetArguments);

        }

        final Wiidget before = wiidget;

        // START WIIDGET
        wiidget = startWiidget(wiidget); // after start wiidget can be null when
        // its not rendered
        ...
        if (null != wiidget) {
            ...
            if (wiidget instanceof Extension) { // extension support

                final Extension extension = (Extension) wiidget;
                extension.processStatements(declarationContext.wiidgetBody().statementDeclaration(), new StatementProcessor() {
                    @Override
                    public void processStatements(final List<StatementDeclarationContext> statementDeclaration) throws ParserException {
                        TemplateProcessor.this.processStatements(statementDeclaration);
                    }
                });
    ...
    /**
     * @param wiidgetResource resource
     * @param arguments       arguments declaration
     * @return wiidget
     * @throws ParserException when cannot create wiidget
     */
    private Wiidget createWiidget(final WiidgetResource wiidgetResource, final WiidgetArgumentsContext arguments) throws ParserException {
        ...
        if (wiidgetResource instanceof FileWiidgetResource) {
            ...
            wiidget = getEngine().createWiidget(ownerView, type, dataMap, true);
    ...
    private Wiidget createExternalWiidget(final ExternalWiidgetResource wiidgetResource, final WiidgetArgumentsContext arguments) throws ParserException {
        ...
        return getEngine().createWiidget(this, WiidgetTemplate.class, data().set(DefaultField.NAME, content).set(WiidgetTemplate.CONTEXT_ATTRIBUTE, data), true);
    ...
    private String getAttributeName(final AttributeNameContext attributeName) throws EvaluationException {

        final TerminalNode stringLiteral = attributeName.StringLiteral();
        if (null == stringLiteral) {
            return attributeName.Identifier().getText();
        }
        return new StringDeclaration(stringLiteral).getContent(getEngine());
    }
     ...
     *
     * @param expression the expression descriptor
     * @return value of expression
     * @throws EvaluationException when expression is illegal or cannot evaluate
     */
    ...
    protected ExpressionEvaluator createExpressionEvaluator() {
        ...
        final EvaluationContext evaluationContext = new EvaluationContext(importContext, getEngine(), this);
     ...
     *
     * @param antlrInputStream ANTLR input
    ...
    private static CompilationUnitContext getCompilationUnitContext(final ANTLRInputStream antlrInputStream) {
        ...
        final WiidgetParser aWiidgetParser = createParser(antlrInputStream);
        final CompilationUnitContext compilationUnit = aWiidgetParser.compilationUnit();

        return compilationUnit;
    }

    public static ExpressionContext getExpressionContext(final String expression) {
        final ANTLRInputStream antlrInputStream = new ANTLRInputStream(expression);
        final WiidgetParser parser = createParser(antlrInputStream);
        return parser.expression(0);
    }

    private static WiidgetParser createParser(final ANTLRInputStream antlrInputStream) {
        ...
        return aWiidgetParser;
    }

    public StringExpressionEvaluation createStringExpressionEvaluation() {
        return new StringExpressionEvaluation(this, createExpressionEvaluator());
    }
