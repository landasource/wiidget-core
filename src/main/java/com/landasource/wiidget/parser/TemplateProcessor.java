...
public class TemplateProcessor extends WiidgetView implements StatementProcessor {
    ...
    protected void transformWiidget(final Wiidget wiidget) {

        final List<TransformRule> transformRules = getEngine().getConfiguration().getTransformRules();
        for (final TransformRule transformRule : transformRules) {

            if (transformRule.match(wiidget)) {
                transformRule.transform(wiidget);
            }
        }
    }
